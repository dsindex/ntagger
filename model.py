...
class GloveDensenetCRF(BaseModel):
    def __init__(self, config, embedding_path, label_path, pos_path, emb_non_trainable=True, use_crf=False):
        ...
        padding = (ks - 1)//2
        self.conv_1 = nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=ks, padding=padding)
        '''
        ...
        self.conv_2 = nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=ks, padding=padding)
        ks = 5
        in_channels = first_num_filters + num_filters
        out_channels = num_filters
        padding = (ks - 1)//2
        self.conv_3 = nn.Conv1d(in_channels=in_channels, out_channels=out_channels, kernel_size=ks, padding=padding)
        ks = 1
        in_channels = emb_dim + num_filters
    ...
    def forward(self, x, tags=None):
        ...
        conv_3 = self.conv_3(torch.cat([conv_1, conv_2], dim=-2))
        conv_3 = F.relu(conv_3)
        # conv_3    : [batch_size, num_filters, seq_size(same)]
        conv_last = self.conv_last(torch.cat([embed_out, conv_3], dim=-2))
